#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Remote Memory Test Launcher  (ngrok Tunnel)
===========================================
Usage:  python run_remote.py
Requires ngrok.exe at C:\\Pictures-task\\ngrok.exe
Auth token must be configured once with:
  ngrok config add-authtoken <token from .ngrok_token>
"""

import os
import sys
import time
import json
import subprocess
import threading

BASE_DIR    = os.path.dirname(os.path.abspath(__file__))
PORT        = 5000
NGROK_EXE   = r'C:\Pictures-task\ngrok.exe'

_public_url = None          # filled by the reader thread


# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

def _url_reader(proc):
    """Read ngrok stderr JSON in a background thread; extract public URL."""
    global _public_url
    for line in iter(proc.stderr.readline, b''):
        text = line.decode('utf-8', errors='replace').strip()
        try:
            obj = json.loads(text)
            if obj.get('msg') == 'started tunnel' and 'url' in obj:
                if _public_url is None:
                    _public_url = obj['url']
        except (ValueError, KeyError):
            pass


def start_flask():
    return subprocess.Popen(
        [sys.executable, os.path.join(BASE_DIR, 'app.py')],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
        cwd=BASE_DIR,
    )


def start_ngrok():
    global _public_url
    _public_url = None
    proc = subprocess.Popen(
        [NGROK_EXE, 'http', str(PORT),
         '--log', 'stderr', '--log-format', 'json'],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
    )
    t = threading.Thread(target=_url_reader, args=(proc,), daemon=True)
    t.start()
    return proc


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main():
    print()
    print('=== Memory Test Remote Launcher (ngrok Tunnel) ===')
    print()

    if not os.path.exists(NGROK_EXE):
        print('ERROR: ngrok.exe not found at: ' + NGROK_EXE)
        print('Download it from https://ngrok.com/download')
        sys.exit(1)

    print('Starting memory test server (Flask) on port {}...'.format(PORT))
    flask_proc = start_flask()
    time.sleep(3)

    print('Starting ngrok Tunnel (this may take ~10 s)...')
    ng_proc = start_ngrok()

    # Wait up to 40 s for the URL
    deadline = time.time() + 40
    while _public_url is None and time.time() < deadline:
        time.sleep(1)

    if _public_url is None:
        print()
        print('ERROR: Timed out waiting for ngrok URL.')
        flask_proc.kill()
        ng_proc.kill()
        sys.exit(1)

    print()
    print('=' * 56)
    print('  PUBLIC URL:  ' + _public_url)
    print('=' * 56)
    print()
    print('Send this link to the participant.')
    print()
    print('They enter:')
    print('  - Participant ID')
    print('  - Session         (Baseline / Psilocybin)')
    print('  - Participant List (1 / 2 / 3 / 4)')
    print('  - Test delay      (24h or 48h)')
    print()
    print('Keep this window open.  Press Ctrl+C to stop.')
    print()

    try:
        while True:
            time.sleep(10)
            if flask_proc.poll() is not None:
                print('Flask stopped -- restarting...')
                flask_proc = start_flask()
                time.sleep(3)
            if ng_proc.poll() is not None:
                print('ngrok stopped -- restarting...')
                ng_proc = start_ngrok()
                deadline2 = time.time() + 30
                while _public_url is None and time.time() < deadline2:
                    time.sleep(1)
                if _public_url:
                    print('New URL: ' + _public_url)
    except KeyboardInterrupt:
        print()
        print('Shutting down...')
        flask_proc.kill()
        ng_proc.kill()
        print('Done.')


if __name__ == '__main__':
    main()
